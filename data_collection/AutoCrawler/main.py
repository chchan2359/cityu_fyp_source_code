...
class AutoCrawler:
    ...
    def download_images(self, keyword, links, site_name, max_count=0):
        path_wording = keyword.replace(":","_")
        path_wording = path_wording.replace("/","_")
        path_wording = path_wording.replace(",","_")
        path_wording = path_wording.replace(".","_")
        path_wording = path_wording.replace(" ","_")
        #print(f'DOWNLOADINGNGNGNG: {path_wording}')

        self.make_dir('{}/{}'.format(self.download_path, path_wording))
        total = len(links)
        success_count = 0
        ...
        for index, link in enumerate(links):
            ...
            try:
                ...
                no_ext_path = '{}/{}/{}_{}'.format(self.download_path.replace('"', ''), path_wording, site_name,
                                                   str(index).zfill(4))
                path = no_ext_path + '.' + ext
                #print(f"[no_ext_path]: [{no_ext_path}]")
                #print(f'[Path] : [{path}]')
    ...
    def download_from_site(self, keyword, site_code):
        ...
        try:
            ...
            path_wording = path_wording.replace(".","_")
            path_wording = path_wording.replace(" ","_")
            #print(f"path_wording:{path_wording}")

            print('Downloading images from collected links... {} from {}'.format(keyword, site_name))
            self.download_images(keyword, links, site_name, max_count=self.limit)
            Path('{}/{}/{}_done'.format(self.download_path, path_wording, site_name)).touch()
    ...
    def do_crawling(self):
        ...
        for keyword in keywords:
            ...
            dir_name = '{}\{}'.format(self.download_path, path_wording)
